...
$ret=($it==1)?imagegif($thumb, $tgtfile): ($it==2)?imagejpeg($thumb, $tgtfile, 90):imagepng($thumb, $tgtfile);

$url = str_replace(" ", "%20", htmlspecialchars("$BASEURL/bitbucket/$filename"));
stderr("Success", "Use the following URL to access the file: <b><a href=\"$url\">$url</a></b><p><a href=/bitbucket-upload.php>Upload another file</a>.<br>Image ". ($width=$newwidth && $height==$newheight ? "doesn't require rescaling":"Image rescaled from $height x $width to $newheight x $newwidth") .'.');
}
